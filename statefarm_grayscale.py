...
#Clear and Make temp_train_fold, temp_valid_fold, cache, subm folders
def _clear_dir(path):
...
#Make training data & validation data (Grayscale)
def generate_split():
    ...
    for label in labels:
        files = glob.glob('input/train/{}/*'.format(label))
        for fl in files:
            origin = cv2.imread(fl)
            gray = cv2.cvtColor(origin, cv2.COLOR_BGR2GRAY)
            if np.random.randint(nfolds) != 1:
                cv2.imwrite('input/temp_train_fold/{}/{}'.format(label, os.path.basename(fl)), gray)
                train_samples += 1
            else:
                cv2.imwrite('input/temp_valid_fold/{}/{}'.format(label, os.path.basename(fl)), gray)
                valid_samples += 1
